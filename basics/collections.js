...
// 1. toString(): Converts the array to a string representation.
...
// 3. push(element1, element2, ...): Adds one or more elements to the end of the array and returns the new length of the array.
...
// 5. unshift(element1, element2, ...): Adds one or more elements to the beginning of the array and returns the new length of the array.
...
// 6. join(separator): Joins all elements of the array into a string, separated by the specified separator.

let joinedString = pysiders.join(" - ")
console.log("Joined string: ", joinedString) // Joined string:  Students - Mentors - Alumni - Instructors

// 7. concat(array1, array2, ...): Merges two or more arrays and returns a new array.
...
// 8. slice(start, end): Returns a shallow copy of a portion of the array into a new array object selected from start to end (end not included).

let slicedArray = pysiders.slice(1, 3)
console.log("Sliced array: ", slicedArray) // Sliced array:  [ 'Mentors', 'Alumni' ]

// 9. splice(start, deleteCount, item1, item2, ...): Changes the contents of an array by removing or replacing existing elements and/or adding new elements in place.
...
// 11. reverse(): Reverses the order of the elements in the array in place and returns the reversed array.

unsortedArray.reverse()
console.log("Reversed array: ", unsortedArray) // Reversed array:  [ 4, 3, 2, 1 ]

// 12. fill(value, start, end): Fills all the elements of an array from a start index to an end index with a static value.

let fillArray = new Array(5).fill(0)
console.log("Filled array: ", fillArray) // Filled array:  [ 0, 0, 0, 0, 0 ]

// 23. map(callback): Creates a new array populated with the results of calling a provided function on every element in the calling array.
